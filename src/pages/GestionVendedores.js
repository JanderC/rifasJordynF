// GestionVendedores.js — RIFAS JORDYN (v6)
// CAMBIOS v6:
// • SIMULTÁNEA: Un vendedor PUEDE tener el mismo número en Serie A y B.
//   Al agregar un número que ya tiene en A → va a B (si libre).
//   Si ya tiene A y B → no permitido.
// • SerieBadge renovado: "Ya tienes A → irá a B", conflictos detallados.
// • InlineNotif: panel de notificaciones visible dentro de los modales.
// • ConflictPanel: bloque de conflictos persistente con dueños por serie.
// • agregarAlPool maneja asignaciones_extra y colisiones_extra (v6).
      ...
      return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          <span style={{ padding: '3px 9px', borderRadius: 6, fontSize: '0.7rem', fontWeight: 700, background: '#fff8e1', border: '1px solid #f0a50060', color: '#b07800', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
            <i className="bi bi-check2-all" style={{ fontSize: '0.65rem' }}></i>
            Ya tienes Serie A y Serie B
          </span>
          <span style={{ fontSize: '0.63rem', color: '#b07800' }}>Sin más espacio para este número.</span>
        </div>
      );
    }
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
        ...
        {info.serie_a?.dueno && (
          <span style={{ fontSize: '0.61rem', color: '#4361ee', display: 'flex', alignItems: 'center', gap: 4 }}>
            <span style={{ background: '#f0f5ff', border: '1px solid #4361ee30', borderRadius: 4, padding: '1px 5px', fontWeight: 700 }}>A</span>{info.serie_a.dueno}
          </span>
        )}
        {info.serie_b?.dueno && (
          <span style={{ fontSize: '0.61rem', color: '#e91e8c', display: 'flex', alignItems: 'center', gap: 4 }}>
            <span style={{ background: '#fff0f5', border: '1px solid #e91e8c30', borderRadius: 4, padding: '1px 5px', fontWeight: 700 }}>B</span>{info.serie_b.dueno}
          </span>
        )}
      </div>
    );
  }
  const esBSerie = info.serie_destino === 'B';
  const esConA   = info.ya_tiene_a;
  const color    = esBSerie ? '#e91e8c' : '#4361ee';
  const bgColor  = esBSerie ? '#fff0f5' : '#f0f5ff';
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      <span style={{ padding: '3px 9px', borderRadius: 6, fontSize: '0.7rem', fontWeight: 700, background: bgColor, border: `1px solid ${color}40`, color, display: 'inline-flex', alignItems: 'center', gap: 5 }}>
      ...
      {esBSerie && info.serie_a?.dueno && !esConA && (
        <span style={{ fontSize: '0.61rem', color: '#4361ee', display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ background: '#f0f5ff', border: '1px solid #4361ee30', borderRadius: 4, padding: '1px 5px', fontWeight: 700 }}>A</span>{info.serie_a.dueno}
        </span>
      )}
...
function InlineNotif({ items, onClear }) {
  ...
  return (
    <div style={{ marginBottom: '1rem' }}>
      ...
      {onClear && (
        <button onClick={onClear} style={{ background: 'none', border: 'none', color: 'var(--jordyn-muted)', fontSize: '0.68rem', cursor: 'pointer', padding: '2px 6px' }}>
          <i className="bi bi-x me-1"></i>Limpiar notificaciones
        </button>
      )}
    </div>
  );
...
// ── ConflictPanel ────────────────────────────────────────────────
function ConflictPanel({ colisiones, titulo = 'Sin espacio — números no asignados' }) {
  if (!colisiones || !colisiones.length) return null;
  return (
    <div style={{ background: '#fff5f5', border: '2px solid #ffaaaa', borderRadius: 12, padding: '10px 14px', marginBottom: '1rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
        {colisiones.map((c, i) => (
          <div key={i} style={{ padding: '5px 8px', background: '#fff', borderRadius: 8, border: '1px solid #ffcccc' }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {c.dueno_a && (
                <span style={{ fontSize: '0.62rem', color: '#4361ee', display: 'inline-flex', alignItems: 'center', gap: 3 }}>
                  <span style={{ background: '#f0f5ff', border: '1px solid #4361ee30', borderRadius: 4, padding: '0 4px', fontWeight: 700, fontSize: '0.6rem' }}>A</span>{c.dueno_a}
                </span>
              )}
              {c.dueno_b && (
                <span style={{ fontSize: '0.62rem', color: '#e91e8c', display: 'inline-flex', alignItems: 'center', gap: 3 }}>
                  <span style={{ background: '#fff0f5', border: '1px solid #e91e8c30', borderRadius: 4, padding: '0 4px', fontWeight: 700, fontSize: '0.6rem' }}>B</span>{c.dueno_b}
                </span>
              )}
            </div>
          </div>
      ...
      <div style={S.modal(560)}>
        <div style={S.header(esEditar ? 'linear-gradient(135deg,#1a7a40,#2ecc71)' : 'linear-gradient(135deg,#0a3d62,#0abfbc)')}>
          <div>
            ...
            <div style={{ fontSize: '0.72rem', opacity: 0.85, marginTop: 2 }}>{esEditar ? categoria.nombre : 'Parcial o Simultánea con Series A y B'}</div>
          </div>
        ...
        <div style={S.body}>
          ...
          <div style={{ marginBottom: '1rem' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem', marginTop: '0.5rem' }}>
              {[
                { key: 'parcial', icon: 'bi-bullseye', label: 'Parcial', accent: '#4361ee', desc: 'Serie única 000-999. Cada número pertenece a un solo vendedor.' },
                { key: 'simultanea', icon: 'bi-lightning-fill', label: 'Simultánea', accent: '#e91e8c', desc: 'Series A y B. Un mismo número puede estar en A y en B (incluso el mismo vendedor puede tener ambas).' },
              ].map(({ key, icon, label, desc, accent }) => (
                <div key={key} onClick={() => !esEditar && setForm(p => ({ ...p, tipo: key }))}
                  style={{ flex: 1, padding: '1rem', borderRadius: 12, cursor: esEditar ? 'default' : 'pointer', border: `2px solid ${form.tipo===key ? accent : 'var(--jordyn-border)'}`, background: form.tipo===key ? `${accent}10` : 'var(--jordyn-bg2)', transition: 'all .15s', opacity: esEditar ? 0.7 : 1 }}>
            ...
            {esEditar && <div className="jd-alert jd-alert-warning mt-2" style={{ fontSize: '0.75rem' }}><i className="bi bi-lock-fill me-1"></i>El tipo no se puede cambiar una vez creada.</div>}
          </div>
      ...
      <div style={S.modal(460)}>
        ...
        <div style={S.body}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem', marginBottom: '1.25rem' }}>
            <div><label className="jd-label">NOMBRE COMPLETO *</label><input className="jd-input" value={form.nombre} onChange={e => setForm(p => ({ ...p, nombre: e.target.value }))} placeholder="Nombre" autoFocus /></div>
...
function ModalNumerosEnCategoria({ categoria, vendedor, onClose, onSaved }) {
  ...
  // v6: maneja asignaciones_extra y colisiones_extra
  const agregarAlPool = async nums => {
    ...
    try {
      ...
      if (r.data.insertados > 0) addNotif(`${r.data.insertados} número(s) agregados al pool.`, 'success');
      if (r.data.asignaciones_extra?.length > 0) {
        r.data.asignaciones_extra.forEach(a => addNotif(`${a.numero} → asignado a Serie ${a.serie} ✓`, 'success'));
        toast.success(`${r.data.asignaciones_extra.length} número(s) asignados a segunda serie`);
      }
      if (r.data.colisiones_extra?.length > 0) r.data.colisiones_extra.forEach(c => addNotif(`${c.numero}: ${c.mensaje}`, 'error'));
      if (r.data.ya_existian?.length > 0 && !esSim) addNotif(`Ya estaban en el pool: ${r.data.ya_existian.join(', ')}`, 'warning');
      if (r.data.insertados === 0 && !r.data.asignaciones_extra?.length && !r.data.colisiones_extra?.length) addNotif('Sin cambios — los números ya estaban y sin serie disponible.', 'warning');
  ...
  const liberarSerie = async asignacion_id => {
    try {
      ...
      addNotif('Serie liberada. El número sigue en el pool.', 'info');
      await cargar(); onSaved?.();
    } catch (err) { toast.error(err.response?.data?.error || 'Error'); }
  };
  ...
  const asignarNumeros = async () => {
    ...
    try {
      ...
      addNotif(d?.error || 'Error asignando', 'error');
      ...
      <div style={S.modal(700)}>
        <div style={S.header(`linear-gradient(135deg,${esSim ? '#7b0050,#e91e8c' : '#0a3d62,#0abfbc'})`)}>
          <div>
            ...
            <div style={{ fontSize: '0.72rem', opacity: 0.85, marginTop: 2 }}>{categoria.nombre} — {totalNumeros} en pool — {conSerie.length} con serie — {sinSerie.length} pendientes</div>
          </div>
          <button onClick={onClose} style={S.closeBtn}><i className="bi bi-x-lg"></i></button>
        </div>
        <div style={S.body}>
          <div className="jd-alert jd-alert-info mb-3" style={{ fontSize: '0.75rem', lineHeight: 1.7 }}>
            ...
            {esSim ? <><strong>Simultánea v6:</strong> Serie A libre → va a A. Si A está ocupada por otro → va a B. <strong>Puedes tener el mismo número en A y B</strong> — agrégalo dos veces para ocupar ambas series.</> : <><strong>Parcial:</strong> Un número solo puede pertenecer a un vendedor.</>}
          </div>
          ...
          <div style={{ background: 'var(--jordyn-bg2)', borderRadius: 10, padding: '0.75rem 1rem', marginBottom: '1rem', border: '1px solid var(--jordyn-border)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.72rem', color: 'var(--jordyn-muted)', fontWeight: 600, marginBottom: 5 }}>
              <span>POOL</span><span>{totalNumeros} definidos — {conSerie.length} con serie — {sinSerie.length} pendientes</span>
            </div>
          ...
          {loading
            ...
            <>
              {sinSerie.length > 0 && (
                <div style={{ background: 'var(--jordyn-bg2)', border: `2px solid ${accent}30`, borderRadius: 14, padding: '1rem', marginBottom: '1rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: preview ? '0.75rem' : 0 }}>
                    <div style={{ fontWeight: 700, fontSize: '0.85rem', color: accent }}><i className="bi bi-lightning-fill me-1"></i>{sinSerie.length} número(s) sin serie</div>
                  ...
                  {loadingPrev && <div style={{ fontSize: '0.72rem', color: 'var(--jordyn-muted)', display: 'flex', alignItems: 'center', gap: 6 }}><span className="jd-spinner" style={{ width: 12, height: 12 }}></span> Calculando preview...</div>}
                  {preview && !loadingPrev && (
                        ...
                        return (
                          <div key={serie} style={{ background: serie==='A' ? '#f0fff8' : '#fff0f5', border: `1.5px solid ${color}40`, borderRadius: 10, padding: '8px 12px' }}>
                            <div style={{ fontSize: '0.7rem', fontWeight: 700, color, marginBottom: 5 }}>
                              <i className="bi bi-check-circle-fill me-1"></i>Serie {serie}: {nums.length} número(s)
                            </div>
              ...
              {totalNumeros === 0 && <div className="jd-alert jd-alert-warning" style={{ fontSize: '0.82rem' }}>Sin números en el pool. Usa las opciones de arriba para agregar.</div>}
            </>
          )}
        </div>
      </div>
...
// ── ModalCrearVendedorEnCategoria (v6) ───────────────────────────
function ModalCrearVendedorEnCategoria({ categoria, vendedoresDisponibles, onClose, onSaved }) {
  ...
  const agregarRango = () => {
    ...
    if (isNaN(ini) || isNaN(fin) || ini > fin || ini < 0 || fin > 999) { toast.error('Rango inválido'); return; }
    const nuevos = [];
    for (let i = ini; i <= fin; i++) { const num = pad3(i); if (!numeros.includes(num)) nuevos.push(num); }
  ...
  const confirmar = async () => {
    ...
    try {
      ...
      const payload = {
        ...(modo === 'existente' ? { vendedor_id: vendedorSel } : { nombre: form.nombre.trim(), cedula: form.cedula||undefined, ...creds }),
        numeros,
      };
            ...
            {['A','B'].map(serie => {
              ...
              return (
                <div key={serie} style={{ marginBottom: '0.75rem', background: serie==='A' ? '#f0f5ff' : '#fff0f5', border: `1.5px solid ${color}30`, borderRadius: 10, padding: '10px 14px' }}>
                  <div style={{ fontSize: '0.68rem', fontWeight: 700, color, textTransform: 'uppercase', marginBottom: 6 }}>
                    <i className={`bi bi-${serie==='A' ? 'bullseye' : 'lightning-fill'} me-1`}></i>Serie {serie} — {nums.length} número(s)
                  </div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
        ...
        <div style={S.body}>
          <div style={{ display: 'flex', borderRadius: 10, overflow: 'hidden', border: '1.5px solid var(--jordyn-border)', marginBottom: '1rem' }}>
            {[['nuevo','Crear nuevo vendedor'],['existente','Vendedor existente']].map(([k,l],i) => (
              <button key={k} onClick={() => { setModo(k); setNumInput(''); }} style={{ flex: 1, padding: '9px 8px', border: 'none', cursor: 'pointer', fontSize: '0.8rem', fontWeight: 600, background: modo===k ? accent : 'transparent', color: modo===k ? '#fff' : 'var(--jordyn-muted)', borderRight: i===0 ? '1.5px solid var(--jordyn-border)' : 'none' }}>{l}</button>
            ))}
          </div>
          ...
          {modo === 'nuevo' ? (
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              {!vendedoresDisponibles.length && <div style={{ fontSize: '0.7rem', color: 'var(--jordyn-muted)', marginTop: 4 }}>Todos los vendedores activos ya están en esta categoría.</div>}
            </div>
          )}

          <div className="jd-alert jd-alert-info mb-3" style={{ fontSize: '0.75rem', lineHeight: 1.6 }}>
            <i className="bi bi-info-circle-fill me-2"></i>
            {esSim ? <><strong>Simultánea:</strong> Serie A primero. Si A está tomada → va a B. <strong>Un mismo vendedor puede tener el mismo número en A y en B.</strong></> : <><strong>Parcial:</strong> Cada número pertenece a un solo vendedor.</>}
          </div>
          ...
          <div style={{ background: 'var(--jordyn-bg2)', border: `2px solid ${accent}25`, borderRadius: 14, padding: '1rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-start', flexWrap: 'wrap', marginBottom: '0.5rem' }}>
                <div>
                  <input className="jd-input" value={numInput}
                    ...
                    placeholder="000-999"
                    style={{ maxWidth: 110, textAlign: 'center', fontWeight: 700, letterSpacing: 2, borderColor: infoInput ? (infoInput.disponible ? accent : '#ffaaaa') : undefined }}
                    maxLength={3} />
                  <div style={{ marginTop: 5, minHeight: 42 }}>
                    {validando ? <span className="jd-spinner" style={{ width: 12, height: 12 }}></span>
                      : numInput.length===3 && infoInput ? <SerieBadge info={infoInput} />
                      : numInput.length===3 && modo==='nuevo' ? <span style={{ fontSize: '0.63rem', color: 'var(--jordyn-muted)' }}>Se validará al guardar</span>
                      : null}
                  </div>
                </div>
                <button className="btn-jordyn" onClick={agregarNumeroManual} disabled={infoInput && !infoInput.disponible}
                  style={{ background: `linear-gradient(135deg,${accent},${accent}cc)`, height: 44 }}>
                  <i className="bi bi-plus-lg me-1"></i>Agregar
                </button>
              </div>
            ...
            {numeros.length > 0 && (
              <div style={{ marginTop: '0.85rem' }}>
                <div style={{ fontSize: '0.65rem', fontWeight: 700, color: 'var(--jordyn-muted)', textTransform: 'uppercase', marginBottom: 5 }}>Lista ({numeros.length} número{numeros.length!==1?'s':''})</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  {numeros.map(num => (
                    <div key={num} style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      ...
                      <button onClick={() => setNumeros(p => p.filter(n => n!==num))} style={{ background: 'none', border: 'none', color: '#e63946', cursor: 'pointer', padding: 0, fontSize: '0.65rem' }}><i className="bi bi-x"></i></button>
                    </div>
                  ))}
                </div>
              </div>
            )}
            {!numeros.length && <div style={{ fontSize: '0.75rem', color: 'var(--jordyn-muted)', fontStyle: 'italic', marginTop: '0.5rem' }}>Opcional — puedes agregar números después de crear el vendedor.</div>}
          </div>
          ...
          <div className="d-flex gap-2">
            <button className="btn-jordyn w-100" onClick={confirmar} disabled={saving} style={{ background: `linear-gradient(135deg,${accent},${accent}cc)` }}>
              {saving ? <><span className="jd-spinner" style={{ width: 15, height: 15 }}></span> Creando...</>
                : <><i className="bi bi-person-check-fill me-1"></i>{modo==='nuevo' ? 'Crear vendedor' : 'Vincular vendedor'}{numeros.length > 0 ? ` + ${numeros.length} número${numeros.length!==1?'s':''}` : ''}</>}
            </button>
            <button className="btn-jordyn-outline" onClick={onClose} style={{ flexShrink: 0, padding: '0 18px' }}>Cancelar</button>
          </div>
        </div>
...
function ModalGestionarCategoria({ categoria, onClose, onSaved }) {
  ...
  const cargar = useCallback(async () => {
    setLoading(true);
    try {
      const [rR, vR] = await Promise.all([API.get(`/categorias-globales/${categoria.id}/vendedores-resumen`), API.get('/vendedores')]);
  ...
  const quitarDeCategoria = async vendedor_id => {
    try {
      ...
      setConfirmQuitar(null); await cargar(); onSaved?.();
    } catch (err) { toast.error(err.response?.data?.error || 'Error'); }
  };
      ...
      <div style={S.modal(980)}>
        ...
        <div style={S.body}>
          <div style={{ background: 'var(--jordyn-bg2)', border: `2px solid ${accent}30`, borderRadius: 14, padding: '1rem', marginBottom: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontWeight: 700, fontSize: '0.88rem', color: accent }}><i className="bi bi-person-plus-fill me-2"></i>Agregar vendedor a esta categoría</div>
          ...
          <div className="jd-alert jd-alert-info mb-3" style={{ fontSize: '0.75rem', lineHeight: 1.6 }}>
            <i className="bi bi-info-circle-fill me-2"></i>
            {esSim ? <><strong>Simultánea:</strong> Serie A primero. Si A está ocupada por otro vendedor → va a B. <strong>Un mismo vendedor puede tener el mismo número en Serie A y en Serie B</strong> (doble serie).</> : <><strong>Parcial:</strong> Un número solo puede pertenecer a un vendedor en esta categoría.</>}
          </div>
          ...
          <div style={S.modal(440)}>
            ...
            <div style={S.body}>
              <p style={{ fontSize: '0.88rem', marginBottom: 16, lineHeight: 1.7 }}>
                Quitar a <strong>"{confirmQuitar.nombre}"</strong> de <strong>"{categoria.nombre}"</strong>. Se eliminan sus números y series en esta categoría. <strong>El vendedor NO se elimina del sistema.</strong>
              </p>
              <div className="d-flex gap-2">
                <button className="btn-jordyn w-100" onClick={() => quitarDeCategoria(confirmQuitar.vendedor_id)} style={{ background: 'linear-gradient(135deg,#b37700,#f0a500)' }}>
          ...
          <div style={S.modal(440)}>
            ...
            <div style={S.body}>
              {confirmDelete.tieneVentas ? (
                <>
                  <div style={{ background: '#fff8e1', border: '1.5px solid #ffd166', borderRadius: 10, padding: '12px 14px', marginBottom: 16, fontSize: '0.82rem', color: '#7a5c00', lineHeight: 1.6 }}><strong>No se puede eliminar</strong> porque tiene ventas registradas.</div>
                  <p style={{ fontSize: '0.82rem', color: 'var(--jordyn-muted)', marginBottom: 20, lineHeight: 1.6 }}>Puedes <strong>desactivarlo</strong> para que no pueda iniciar sesión, conservando su historial.</p>
                  <div className="d-flex gap-2">
                    <button className="btn-jordyn w-100" onClick={desactivarVendedor} style={{ background: 'linear-gradient(135deg,#b37700,#f0a500)' }}><i className="bi bi-pause-circle me-1"></i>Desactivar vendedor</button>
                    <button className="btn-jordyn-outline" onClick={() => setConfirmDelete(null)} style={{ flexShrink: 0, padding: '0 16px' }}>Cancelar</button>
                  </div>
                </>
              ) : (
                <>
                  <p style={{ fontSize: '0.88rem', marginBottom: 16, lineHeight: 1.7 }}>Eliminar definitivamente a <strong>"{confirmDelete.nombre}"</strong>. Esta acción no se puede deshacer.</p>
                  <div className="d-flex gap-2">
                    <button className="btn-jordyn-danger w-100" onClick={eliminarDelSistema}><i className="bi bi-trash3-fill me-1"></i>Eliminar definitivamente</button>
                    <button className="btn-jordyn-outline" onClick={() => setConfirmDelete(null)} style={{ flexShrink: 0, padding: '0 16px' }}>Cancelar</button>
                  </div>
                </>
              )}
      ...
      {confirmDelCat && (
        <div onClick={e => e.target===e.currentTarget && setConfirmDelCat(null)} style={S.overlay}>
          <div style={S.modal(420)}>
            ...
            <div style={S.body}>
              <p style={{ fontSize: '0.88rem', marginBottom: 16, lineHeight: 1.7 }}>
                Eliminar <strong>"{confirmDelCat.nombre}"</strong>. Se eliminarán todos los vendedores y números de esta categoría. <strong>Los vendedores no se eliminan del sistema.</strong>
              </p>
              <div className="d-flex gap-2">
